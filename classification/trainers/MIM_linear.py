...
class CustomCLIP(nn.Module):
    ...
    def __init__(self, cfg, classnames):
        super().__init__()
        # model = HiViT(
        #     embed_dim=384, depths=[1, 1, 10], num_heads=6, stem_mlp_ratio=3., in_chans=3, mlp_ratio=4., num_classes=len(classnames),
        #     ape=True, rpe=False, norm_layer=partial(nn.LayerNorm, eps=1e-6))
        # model = HiViT(
        #     embed_dim=384, depths=[2, 2, 20], num_heads=6, stem_mlp_ratio=3., in_chans=3, mlp_ratio=4., num_classes=len(classnames),
        #     ape=True, rpe=False, norm_layer=partial(nn.LayerNorm, eps=1e-6))
        model = HiViT(
            embed_dim=512, depths=[2, 2, 20], num_heads=8, stem_mlp_ratio=3., in_chans=3, mlp_ratio=4., num_classes=len(classnames),
            ape=True, rpe=False, norm_layer=partial(nn.LayerNorm, eps=1e-6))
        # model = HiViT(
        #     embed_dim=768, depths=[4, 4, 36], num_heads=12, stem_mlp_ratio=3., in_chans=3, mlp_ratio=4., num_classes=len(classnames),
        #     ape=True, rpe=False, norm_layer=partial(nn.LayerNorm, eps=1e-6))

        checkpoint = torch.load('checkpoint-200.pth', map_location='cpu')
